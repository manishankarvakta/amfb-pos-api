...
// }else{
//     uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.rfbl8.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
// }
...
async function run() {
  try {
    ...
    const posDB = client.db("amfb-pos");
    ...
    app.get("/price-table", async (req, res) => {
      ...
      const query = {};
      const cursor = priceTableCollection.find(query);
      if (page || size) {
        inventory = await cursor
          .skip(page * size)
          .limit(size)
    ...
    // get One price-table
    app.get("/price-table/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: ObjectId(id) };

      const inventory = await priceTableCollection.findOne(query);
      res.send(inventory);
    });

    // update / put price-table
    app.put("/price-table/:id", async (req, res) => {
      const id = req.params.id;
      const inventory = req.body;
      const filter = { _id: ObjectId(id) };
      ...
      const result = await priceTableCollection.updateOne(
        filter,
        updateInventory,
        option
      );
    ...
    // delete price-table
    app.delete("/price-table/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: ObjectId(id) };
      const result = await priceTableCollection.deleteOne(query);
      res.send(result);
    });
  } finally {
    // client.close();
